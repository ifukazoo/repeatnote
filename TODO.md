...
**理由**: RepeatNote は localhost のみでアクセスする個人用アプリのため、外部からの攻撃経路がなく、これらの対策の実用的な価値がない。
...
- 旧 Cloudflare ファイル（`worker/`, `wrangler.jsonc`, `tsconfig.worker.json`, `schema.sql`）を削除
...
**最終更新**: 2026-05-13
**現在の状態**: Hono サーバー（port 3001）でフロントエンド + API を一本化、包括的テストスイート完備（92テスト）、プロジェクトルート整理完了（frontend/ / server/ / mcp/ / scripts/ / plans/）、プロダクション運用可能
