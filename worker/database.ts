...
export async function getItems(db: D1Database): Promise<Item[]> {
  const result = await db.prepare(
    'SELECT * FROM items ORDER BY next_review ASC, created_at DESC'
  ).all();
...
export async function createItem(db: D1Database, data: CreateItemData): Promise<Item> {
  ...
  const result = await db.prepare(`
    ...
    VALUES (?, ?, ?, 1, 2.5, 0)
    RETURNING *
  `).bind(data.content, now, now).first();
...
export async function reviewItem(db: D1Database, id: number, quality: number): Promise<Item | null> {
  // まず現在のアイテム情報を取得
  const currentItem = await db.prepare('SELECT * FROM items WHERE id = ?').bind(id).first() as unknown as Item | null;
...
function calculateNextReview(item: Item, quality: number): {
  nextReview: string;
  intervalDays: number;
  easeFactor: number;
} {
