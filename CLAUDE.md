...
- `npm run dev` - Start development server with Vite HMR (port 5173)
...
- `npm run preview` - ビルド済みファイルをローカルホスティング（port 4173）。コードを変更した場合は先に `npm run build` が必要

### API サーバー（`server/` ディレクトリ）

- `cd server && npm start` - サーバー起動（tsx で `src/index.ts` を実行）
- `cd server && npm run dev` - 開発用（tsx watch モード）
- `cd server && npm run test:run` - サーバー側テストのみ実行

## Obsidian Vault Management

Data is stored as Markdown files in the Obsidian vault at `/Users/ki/Obsidian/MyVault/repeatnote/`.
...
# 画像ファイル確認
ls /Users/ki/Obsidian/MyVault/repeatnote/attachments/
```

### データ移行（Cloudflare → Obsidian）
...
詳細な実行手順は `plans/migration-runbook.md` を参照。
...
- **`src/index.ts`**: Hono アプリのエントリーポイント。CORS・ルーティング・エラーハンドリング・Node.js サーバー起動
...
- **API Communication**: `src/api.ts` → `http://localhost:3001/api` (Hono API Server)
- **Data Storage**: Obsidian vault の `.md` ファイル（Frontmatterに SM-2 メタデータ）
- **Image Storage**: vault の `attachments/` フォルダ（Obsidian で直接参照可能）
- **Image URL**: フロントエンドは `getImageUrl(filename)` → `http://localhost:3001/api/images/{filename}` を使用
...
5. フロントエンドを起動: `npm run dev`（または `npm run preview`）
6. ブラウザで `http://localhost:5173`（dev）または `http://localhost:4173`（preview）にアクセス

### ローカルホスティング（自動起動）

Mac ログイン時に API サーバーとフロントエンドを自動起動する launchd サービスを設定済み。

**フロントエンド plist**: `~/Library/LaunchAgents/com.ifukazoo.repeatnote.plist`（port 4173）

**API サーバー plist**: `~/Library/LaunchAgents/com.ifukazoo.repeatnote-api.plist`（port 3001）
...
```bash
npm run build
launchctl unload ~/Library/LaunchAgents/com.ifukazoo.repeatnote.plist
launchctl load ~/Library/LaunchAgents/com.ifukazoo.repeatnote.plist
```
...
- Hono API サーバー（port 3001）が起動していないとフロントエンドは動作しない
- `npm run dev`（port 5173）・`npm run preview`（port 4173）いずれも同一の本番 Vault に接続する
...
Application fully functional with Hono API Server + Obsidian Local REST API as storage backend:
- ✅ Spaced repetition learning system with SM-2 algorithm（サーバー側で計算）
- ✅ Complete image upload/edit/delete functionality（vault attachments/）
- ✅ Review-first UI with collapsible add form
- ✅ Text search filter for narrowing down the item list
- ✅ Hono API サーバー（port 3001）によるローカル API 層
- ✅ Obsidian Local REST API integration（Cloudflare 完全廃止）
- ✅ Component refactoring: split into hooks + 6 components
- ✅ Markdown viewer with react-markdown (item display + edit preview tab)
- ✅ CSS整理: 共有スタイルを shared.css に集約
...
**Worker テスト (`worker/index.test.ts`)**: 旧 Worker コードのテスト (17 tests)（参照用）

### Test Commands
- `npm test` - Watch mode for development
- `npm run test:run` - Single run for CI/automation（全テスト: フロントエンド + サーバー）
- `npm run test:ui` - Interactive UI mode
- `cd server && npm run test:run` - サーバー側テストのみ
- `npx vitest run src/test/<filename>.test.ts` - 単一テストファイルを実行

**Total: 109 tests** covering SM-2 algorithm, Obsidian parser, API routes, API client layer, validation, and UI components.

### テスト環境の方針

テスト環境は jsdom（ブラウザシミュレーション）とNode.js の 2 環境を使い分け。
- フロントエンドテスト: jsdom（`vitest.config.ts`）
- サーバーテスト: Node.js（`server/vitest.config.ts`）

Obsidian クライアント（`server/src/obsidian/client.ts`）は `vi.mock` でモックし、実際の HTTP 通信は行わない。

**Obsidian REST API の実動作確認方法**: Obsidian と API サーバーを起動した状態で `npm run dev` を実行し、ブラウザで手動確認する。
...
- **Test-driven development**: ビジネスロジック（SM-2アルゴリズム、Obsidianパーサー、API ルートなど）はTDDで開発する（テストを先に書いてから実装）。UIコンポーネントはモノリシックな `App.tsx` の制約上テストファーストが難しいため、実装後にテストを追加する。変更後は `npm test` でテストを確認し、`npm run dev` で手動UIテストをする。
...
- **Item ID**: UUID 文字列（`crypto.randomUUID()`）。旧 D1 の数値 ID とは非互換。
